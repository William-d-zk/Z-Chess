 ...
 * @author william.d.zk
 */
public class FactoryProcessor implements IAnnotationProcessor {
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    if (!roundEnv.processingOver()) {
      ...
      _Logger.info(
          "factory processor ==============================================================");
      roundEnv
          .getElementsAnnotatedWith(ISerialFactory.class)
          .forEach(
              element -> {
                String pkgName =
  ...
  private void createSwitchBuild(Trees trees, Element element, Context context, int parent) {
    ...
    tree.accept(
        new SwitchBuilderTranslator(
            context, _ZProcessor.getProcessorContext().getChildren(parent)));
  }
}
