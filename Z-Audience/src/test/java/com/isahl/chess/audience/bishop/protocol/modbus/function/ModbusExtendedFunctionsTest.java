...
class ModbusExtendedFunctionsTest {
    ...
    void testDiagnosticsSubfunction_fromCode() {
        ...
        assertEquals(DiagnosticsSubfunction.RESTART_COMMUNICATIONS, DiagnosticsSubfunction.fromCode(0x0001));
        assertEquals(DiagnosticsSubfunction.RETURN_DIAGNOSTIC_REGISTER, DiagnosticsSubfunction.fromCode(0x0002));
    }
    
    @Test
    void testDiagnostics_queryData() {
        byte[] inputData = new byte[]{0x12, 0x34};
        byte[] result = ModbusExtendedFunctions.diagnostics(DiagnosticsSubfunction.QUERY_DATA, inputData);
    ...
    void testDiagnostics_restartCommunications() {
        byte[] result = ModbusExtendedFunctions.diagnostics(
    ...
    void testReadWriteMultipleRegisters() {
        ...
        int[] result = ModbusExtendedFunctions.readWriteMultipleRegisters(
            registers,
            0, 3,      // Read 3 registers from address 0
