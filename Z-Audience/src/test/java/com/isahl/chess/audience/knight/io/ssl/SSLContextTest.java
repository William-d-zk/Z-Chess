...
public class SSLContextTest {
  ...
  void testSSLContextInitialization() {
    ...
    assertEquals("TLSv1.2", config.getProviderSslContext().getProtocol(), "应该使用 TLSv1.2 协议");
  }
  ...
  void testSSLBufferSizes() {
    ...
    javax.net.ssl.SSLSession session =
        config.getProviderSslContext().createSSLEngine().getSession();
  ...
  void testCertificateInfo() {
    ...
    assertTrue(java.util.Arrays.asList(supportedProtocols).contains("TLSv1.2"), "应该支持 TLSv1.2");

    // 获取启用的协议
    String[] enabledProtocols = engine.getEnabledProtocols();
    assertTrue(java.util.Arrays.asList(enabledProtocols).contains("TLSv1.2"), "应该启用 TLSv1.2");
  }
