...
public class CertificateWatcherTest {
  ...
  void testCreateWatcher() throws IOException {
    watcher =
        new CertificateWatcher(
            keyStorePath.toString(), trustStorePath.toString(), v -> reloadCount.incrementAndGet());
  ...
  void testStartStop() throws IOException {
    watcher = new CertificateWatcher(keyStorePath.toString(), null, v -> {});
  ...
  void testDetectKeyStoreChange() throws Exception {
    ...
    // 修改 KeyStore 文件
    Files.write(keyStorePath, "updated-keystore".getBytes(), StandardOpenOption.TRUNCATE_EXISTING);
  ...
  void testDebounce() throws Exception {
    ...
    // 快速多次修改文件
    for (int i = 0; i < 5; i++) {
      Files.write(keyStorePath, ("update-" + i).getBytes(), StandardOpenOption.TRUNCATE_EXISTING);
