 ...
 *
 * <p>注意：需要 jSerialComm 库支持 添加依赖：com.fazecast:jSerialComm:[2.0,3.0.0)
 *
 * <p>使用示例:
 *
 * <pre>
...
public class ModbusRtuOverSerial {
  ...
  public void open() throws IOException {
    ...
    serialPort.open();
    LOG.info(
        "Modbus RTU over Serial opened: {} @ {}bps", config.getPortName(), config.getBaudRate());
  }
  ...
  public ModbusResponse sendRequest(ModbusRequest request) throws IOException {
    ...
    while (totalBytesRead < 8 && (System.currentTimeMillis() - startTime) < transactionTimeoutMs) {
