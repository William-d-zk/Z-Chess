...
public abstract class ZControl extends ZProtocol implements IControl<ZContext>, IDuplicate {
  protected static final int attribute_rsv_msg_id = 0x80;
  protected static final int attribute_sub_content = 0x10;
  protected static final int attribute_version_bits_left = 5;
  protected static final int attribute_version_mask = 3 << attribute_version_bits_left;
  ...
  public ZControl() {
    ...
    mFrameHeader |=
        (byte)
            (ZFrame.frame_op_code_ctrl | (level().getValue() << ZFrame.frame_op_code_qos_bit_left));
  }
  ...
  public ZControl withSub(IoSerial subContent) {
    ...
    if (subEncoded.capacity() > 0) {
      mPayload = subEncoded.array();
    }
    return this;
  }
  ...
  protected void addCrc(ByteBuf output) {
    output.putInt(
        CryptoUtil.crc32(
            output.array(), output.writerMark(), output.writerIdx() - output.writerMark()));
  }
