...
/**
 * Modbus TLS 客户端 (完整异步实现) 支持 TLS 1.2 和 TLS 1.3 加密传输
 *
 * <p>Phase 3: TLS 加密数据传输
 */
  ...
  private void doHandshake() throws Exception {
    ...
    while (handshakeStatus != SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING
      ...
      if (handshakeStatus == SSLEngineResult.HandshakeStatus.NEED_UNWRAP) {
        ...
        channel
            .write(java.nio.ByteBuffer.wrap(encryptedData))
            .get(timeoutMs, TimeUnit.MILLISECONDS);
        handshakeStatus = result.getHandshakeStatus();
      ...
      if (bytesRead > 0) {
        ...
        if (unwrapResult.getStatus() == SSLEngineResult.Status.OK && appIn.position() > 0) {
          ...
          return new ModbusResponse(
              response.getUnitId(), response.getFunction(), response.getData());
        }
