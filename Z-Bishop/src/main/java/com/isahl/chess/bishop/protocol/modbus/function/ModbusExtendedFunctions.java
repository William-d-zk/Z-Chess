...
/** Modbus 扩展功能码处理器 支持 Phase 2 功能码：0x07, 0x08, 0x11, 0x16, 0x17 */
public class ModbusExtendedFunctions {

  private ModbusExtendedFunctions() {}

  /**
   * 0x07 Read Exception Status 读取从站的异常状态
   *
   * @param unitId 从站地址
   * @return 异常状态字节
   */
  ...
  public enum DiagnosticsSubfunction {
    ...
    public static DiagnosticsSubfunction fromCode(int code) {
      ...
      throw new IllegalArgumentException(
          "Unknown diagnostics subfunction: 0x" + Integer.toHexString(code));
    }
  }
  ...
  /**
   * 0x11 Report Server ID 报告服务器 ID
   *
   * @param unitId 从站地址
   * @return 服务器 ID 字节数组
   */
   ...
   *
   * <p>AND 掩码和 OR 掩码的计算公式： 新值 = (当前值 AND AND 掩码) OR (OR 掩码 AND (NOT AND 掩码))
   *
