...
public class ModbusResponse extends ModbusMessage {
    ...
    private ModbusResponse(int unitId, ModbusFunction function, byte[] data, int exceptionCode, int functionCodeOverride) {
        super(unitId, function, data);
        this.exceptionCode = exceptionCode;
    ...
    public static ModbusResponse exception(int unitId, ModbusFunction function, int exceptionCode) {
        ...
        return new ModbusResponse(unitId, ModbusFunction.EXCEPTION, new byte[]{(byte) exceptionCode}, exceptionCode, exceptionFunctionCode);
    }
}
